...
int velodyneHDL32E::packageParse(char buf[1206])
{
    ...
    static float   rotangle_err=-7.0f;
    static float   yoffset=3.65f;
	pointType tempPoint;
	...
	for (int i = 0; i < 12; i++)
	{
		...
		for (int j = 0; j < 16; j++)
		{
			...
			if (distance > 1.0 && tempData.size() < dataSizeMax)
			{
				...
				tempPoint.x = float(distance*cos(verticleAngle*Ang2Rad)*sin((rotAngle+rotangle_err)*Ang2Rad));
				tempPoint.y = float(distance*cos(verticleAngle*Ang2Rad)*cos((rotAngle+rotangle_err)*Ang2Rad))+yoffset;
