...
/*
  ...
  src/LookupPOI ../data/ 31.2931 121.2028 31.2832 121.2164 amenity_hospital amenity_building
...
int main(int argc, char* argv[])
{
    ...
    for (const auto &way :ways) {
        ...
        std::stringstream ss;
        ss<<wayCount;
        std::string wayName= "./osm/GPSWay"+ss.str()+".txt";
       // outFile.open(wayName.c_str());
        osmscout::Way temp=*way;
        for(int k=0;k<way->nodes.size();k++)
        {
            osmscout::GeoCoord coord=way->nodes[k].GetCoord();
            outFile<<0<<"\t"<<"[GPS]"<<"\t"<<0<<"\t"<<coord.GetLat()<<"\t"<<coord.GetLon()<<"\t"<<0<<"\t"<<0<<"\t"<<0<<"\t"<<0<<"\t"<<0<<"\t"<<std::endl;
        }
    ...
    for (const auto &area : areas) {
        ...
        for(int kk=0;kk<area->rings.size();kk++)
        {
            for(int k=0;k<area->rings[kk].nodes.size();k++)
            {
                osmscout::GeoCoord coord=area->rings[kk].nodes[k].GetCoord();
                outFile<<0<<"\t"<<"[GPS]"<<"\t"<<0<<"\t"<<coord.GetLat()<<"\t"<<coord.GetLon()<<"\t"<<0<<"\t"<<0<<"\t"<<0<<"\t"<<0<<"\t"<<0<<"\t"<<std::endl;
            }
        }
