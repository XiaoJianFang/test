...
typedef struct
{
    ...
    // Map scale (m/cell)
    double scale;//Õ€žñŸ«¶È
    int submapscale;//×ÓµØÍŒŽóÐ¡
    // Map dimensions (number of cells)
    int size_x, size_y;
    int submapindexsize_x, submapindexsize_y;//×ÓµØÍŒ·¶Î§
    int submapsize;
    int submaplength;
    // The map data, stored as a grid
    map_cell_t *cells; //ÆÕÍšµãÔÆŽæŽ¢¿ÕŒä

    int * submapindex;//×ÓµØÍŒindex ŽæŽ¢Õ€žñ
...
#define SUBMAP_VALID(map, i, j) ((i >= 0) && (i < map->submapscale) && (j >= 0) && (j < map->submapscale))
#define SUBMAPINDEX_VALID(map, i, j) ((i >= 0) && (i <map->submapindexsize_x) && (j >= 0) && (j <map->submapindexsize_y))

// Compute the cell index for the given map coords.
#define MAP_INDEX(map, i, j) ((i) + (j) * map->size_x)
#define SUBMAP_INDEX(map, i, j) ((i) + (j) * map->submapscale)
#define SUBMAPINDEX_INDEX(map, i, j) ((i) + (j)* map->submapindexsize_x )
