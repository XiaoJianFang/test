...
void Test(sensor_msgs::Image & lidarimage, cv::Mat img, pcl::PointCloud<pcl::PointXYZ> scan, cv::Mat P, int num_of_markers, int MAX_ITERS)
{

	ROS_INFO_STREAM("iteration number: " << iteration_count << "\n");
	ROS_INFO_STREAM("cloud number: " << scan.size() << "\n");
			...
			if(iteration_count == 0)
			{
				...
				while(collected != LINE_SEGMENTS[q])
				{
					...
					/*	for( int j = 0; j < polygon.size()-1; j++ )
						{
							cv::line(image_edge_laser, polygon[j], polygon[j+1], cv::Scalar(0, 255, 0));
							cv::imshow("cloud", image_edge_laser);
						}*/
				}
