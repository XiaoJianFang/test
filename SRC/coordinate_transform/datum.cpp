...
my_Ellipse* standard_ellipse(EllipseID id)
{
  return copy_ellipse(_precompiled_ellipse(id));
}

my_Ellipse* copy_ellipse(const StandardEllipse* e)
{
  my_Ellipse* eout = (my_Ellipse*)malloc(sizeof(my_Ellipse));
...
my_Ellipse* copy_ellipse(const my_Ellipse* e)
{
  my_Ellipse* eout = (my_Ellipse*)malloc(sizeof(my_Ellipse));
  eout->name    = (char*)malloc(strlen(e->name)+1);
  eout->id_code = (char*)malloc(strlen(e->id_code)+1);
...
void free_ellipse(my_Ellipse* e)
{
  free(e->name);
  free(e->id_code);
  free(e);
